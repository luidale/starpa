...
class cluster():
    ...
    def cluster(self,settings,first_task):
        ...
        f_info = open(os.path.join(settings["--output"],"cluster","pp.info"),"w")
    ...
    def get_total_reads(self,settings,library,first_task):
        ...
        if first_task in {"cluster", "identify"}:
            total_reads_file = os.path.normpath(os.path.join(settings["--input"],\
                                                ...
                                                library+"_pseudoSE_report.info"))
        else:
            total_reads_file = os.path.join(settings["--output"],"pseudoSE","pseudoSEinfo",\
                                  library+"_pseudoSE_report.info")
        try:
...
##            #f_html.write("<P>Other genomic loci with partial sequence match: NA\n")
        ...
        for i, pp in enumerate(sorted(combined_pp_dict)):
            ...
            for pp_test in sorted(combined_pp_dict)[i+1:]:
                ...
                if overlap_len != 0:
                    ...
                    if min((pp[2]-pp[1])*settings["overlap_fraction"],\
                           (pp[2]-pp[1])-settings["non_overlap"]) <= overlap_len:
                        ##enough overlap from length of tested
                        if min((pp_test[2]-pp_test[1])*settings["overlap_fraction"],\
                               (pp_test[2]-pp_test[1])-settings["non_overlap"]) <= overlap_len:
    ...
    def write_pp_data(self,pp_list,file_name,combined_pp_dict):
        ...
        f_out_BED = open(file_name[:-4]+".library_info","w")
        for pp in pp_list:
...
##            yield (headerStr, seq)
    ...
    def test_errors(self,settings):
        ...
        Check errors
        '''
        info_file = os.path.join(settings["--output"],"cluster","pp.info")
