...
from sort import sam_sort
from pseudoSE import pseudoSE
    ...
    for task in tasks_to_run:
        if task == "trim":
            schema = Schema({
                ...
                "trim_threads": And(Use(int),(lambda s: s > 0), \
                                   error="'trim_cores' should be positive integer"),
...
def check_input_files(args):
    ...
    if args["--start"] == "trim":
        ...
        for file in os.listdir(args["--input"]):
            ...
            sys.exit('Suitable files (ending with "'+\
                         args["identify"]["identify_input_file_suffix_SE"]+\
                         '") missing in input folder')
        ...
        info_files = os.listdir(os.path.normpath(os.path.join(args["--input"],\
                                    args["cluster"]["cluster_pseudoSE_location"])))
        for library in args["libraries"]:
            if library+"_pseudoSE_report.info" not in info_files:
                sys.exit("File "+library+"_pseudoSE_report.info missing from "+
